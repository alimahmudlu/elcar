...
const ConnectorsAndAccessories = async () => {
  const t = await getTranslations("connectors-and-accessories");

  const filterObj = { section: "accessories" };
  ...
  const { data: connectorsAndAccessories } = await fetchData(url);
  return (
    <section className="container my-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mt-6">
        {connectorsAndAccessories?.map((product: ProductType) => (
          <ProductItem
            key={product._id}
            product={product}
...
export default ConnectorsAndAccessories;
