...
const Page = async () => {
  ...
  const categories = await getCategories(CategoryEnum.Car);
  const brands = await getBrands(CategoryEnum.Car);
  const characteristic = await getCharacteristic("vehicles");
  const characteristicOptions = await getCharacteristicOptions("vehicles", [
    ...characteristic?.map((c: ChildCharacteristic) => c._id),
  ]);
  const models = await getModels("vehicles");
    ...
    <section className="container max-md:max-w-[90%] mt-20">
      <div className="mb-8">
        ...
        <p className="mt-4 text-subtitle text-sm md:text-base dark:text-secondary-foreground">
          {t("pages-content.electric-vehicles.subtitle")}
        </p>
      </div>
