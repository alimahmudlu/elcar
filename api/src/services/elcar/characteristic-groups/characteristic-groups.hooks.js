...
module.exports = {
    before: {
        all: [],
        find: [ iff(allowForSite()).else(authenticate('jwt'), checkAbilities(), authorize({ adapter: 'feathers-mongoose' })), setLean(), cache(cacheMap) ],
    ...
    after: {
        ...
        remove: [
            deleteSubItems({
                modules: [
                    { key: 'group', service: 'characteristic' },
                    { key: 'group', service: 'characteristic_options' },
                ]
            })
        ]
    },
