...
module.exports = {
    before: {
        ...
        create: [ authenticate('jwt'), checkAbilities(), authorize({ adapter: 'feathers-mongoose' }), beforeContentSave({title: 'name'}) ],
        update: [ authenticate('jwt'), checkAbilities(), authorize({ adapter: 'feathers-mongoose' }), beforeContentSave({title: 'name'}) ],
        patch: [ authenticate('jwt'), checkAbilities(), authorize({ adapter: 'feathers-mongoose' }), beforeContentSave({title: 'name'}) ],
    ...
    after: {
        ...
        get: [ iff(allowForSite(), setLanguage()), cache(cacheMap) ],
        create: [],
        update: [],
        patch: [],
        remove: []
