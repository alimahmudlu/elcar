...
const getAllowedApps = ()=> {
    return context => {
        ...
        if(user.currentAccess.role.key !== 'root'){
            const ids = user.currentAccess?.permissions.map(per => per.module._id);
...
module.exports = {
    before: {
        all: [],
        find: [ iff(allowForSite(), setLean()).else(authenticate('jwt'), getAllowedApps()), setLean(), sortData(), cache(cacheMap) ],
